...
export async function PATCH(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const allowed = ["description", "discountType", "discountValue", "maxUses", "minAmount", "applicablePlans", "isActive", "expiresAt"];
    const data: Record<string, unknown> = {};
    for (const k of allowed) {
      if (k in body) data[k] = k === "expiresAt" && body[k] ? new Date(body[k]) : body[k];
    }
