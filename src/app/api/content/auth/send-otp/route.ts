...
export async function POST(req: NextRequest) {
  try {
    ...
    const rl = await rateLimit(`content-otp-send:${ip}`, 5, 60 * 1000);
    if (!rl.allowed) {
      return Response.json({ error: "تعداد درخواست بیش از حد مجاز است." }, { status: 429 });
    }
    ...
    const phoneRl = await rateLimit(`content-otp-phone:${phone}`, 1, 60 * 1000);
    if (!phoneRl.allowed) {
    ...
    await prisma.otpCode.create({ data: { phone, code, purpose: "content-login", expiresAt } });

    const sent = await sendOtpSms(phone, code);
    if (!sent) return Response.json({ error: "خطا در ارسال پیامک." }, { status: 500 });
