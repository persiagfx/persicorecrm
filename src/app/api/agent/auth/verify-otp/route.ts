...
export async function POST(req: NextRequest) {
  try {
    ...
    const rl = await rateLimit(`agent-otp-verify:${ip}`, 10, 15 * 60 * 1000);
    if (!rl.allowed) {
    ...
    let user = await prisma.agentUser.findUnique({ where: { phone } });
    if (!user) {
      user = await prisma.agentUser.create({
        data: { name: "کاربر", phone, plan: "FREE" },
      });
    }
    if (!user.isActive) return unauthorized();

    const token = jwt.sign({ userId: user.id, plan: user.plan, isAgentUser: true }, JWT_SECRET, { expiresIn: "30d" });
